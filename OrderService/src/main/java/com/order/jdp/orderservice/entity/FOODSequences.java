...
@Data
@Entity
@Table(name = "FOOD_SEQUENCES", schema = "dev_teja")
@NoArgsConstructor
@AllArgsConstructor
public class FOODSequences implements Serializable, Cloneable {
    private static final long serialVersionUID = 1L;

    @Id
    @Column(name = "TABLE_NAME", length = 128, nullable = false)
    private String tableName; // Assuming TABLE_NAME is the primary key
    ...
    @Override
    public FOODSequences clone() {
        try {
            FOODSequences clone = (FOODSequences) super.clone();
            return clone;
